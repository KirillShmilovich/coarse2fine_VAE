...
class C2FDataSetCLN(Dataset):
    ...
    def __getitem__(self, item):
        ...
        aa = self.noisify(self.coords_aa[trj_idx][frame_idx] @ R.T) + noise
        for i in range(1, self.n_frames):
            aa = np.concatenate(
                (aa, self.noisify(self.coords_aa[trj_idx][frame_idx + i] @ R.T) + noise)
            )

        aa_vox = voxelize_gauss(aa, self.sigma, self.grid)

        cg = self.noisify(self.coords_cg[trj_idx][frame_idx] @ R.T) + noise
        for i in range(1, self.n_frames):
            cg = np.concatenate(
                (cg, self.noisify(self.coords_cg[trj_idx][frame_idx + i] @ R.T) + noise)
            )
...
class C2FDataSet3(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for i in range(1, self.n_frames):
            aa = np.concatenate((aa, self.noisify(self.coords_aa[item + i] @ R.T)))

        aa_vox = voxelize_gauss(aa, self.sigma, self.grid)

        cg = self.noisify(self.coords_cg[item] @ R.T) + noise
        for i in range(1, self.n_frames):
            cg = np.concatenate((cg, self.noisify(self.coords_cg[item + i] @ R.T)))
...
class C2FDataSet2(Dataset):
    def __init__(
        ...
        self.system = self.forcefield.createSystem(
            self.pdb.topology, nonbondedMethod=app.NoCutoff, constraints=mm.app.HBonds
        )
...
def setup_loaders(aa_traj, cg_traj, cfg: Config, log):
    log.debug(f"AA trajectory with {aa_traj.n_frames} frames, {aa_traj.n_atoms} atoms")
    log.debug(f"CG trajectory with {cg_traj.n_frames} frames, {cg_traj.n_atoms} atoms")
    ...
    log.debug(f"Using {n_train} frames for training, the remainder for validation.")
    ...
    ds_val = C2FDataSet(
        aa_coords_val, cg_coords_val, 0, cfg.data.lagtime, False, cfg.grid
    )
